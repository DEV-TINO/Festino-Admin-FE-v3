        ...
        <div className='flex justify-between pt-[100px] min-w-[350px] pb-[40px]'>
          <div className='flex items-center gap-4'>
            ...
            <div className='text-primary-800 text-xl md:text-2xl font-semibold'>
              {boothInfo?.adminName} 부스 정보
            </div>
          </div>
          <button
            className='is-button font-semibold w-[60px] h-[35px] rounded-xl text-sm flex items-center justify-center text-white lg:text-md bg-primary-800 cursor-pointer select-none hover:bg-primary-900 disabled:bg-gray-300 disabled:cursor-not-allowed'
            type='button'
            onClick={() =>
              isBoothOwner
                ? handleClickBoothEdit()
        ...
        <div className='bg-white rounded-2xl w-full h-auto px-4 py-4 pb-12 gap-10 lg:py-[60px] lg:px-[60px] lg:gap-[60px] flex flex-col text-secondary-700 shadow-xs'>
            ...
            <div className='w-full bg-primary-800-light-3 h-auto rounded-2xl px-[40px] py-[40px] border-1 border-primary-800-light-16'>
              <div className='w-full bg-primary-800-light-8 h-full rounded-2xl border border-primary-800-light-16 grid grid-cols-[110px_1fr] grid-rows-[80px_80px_minmax(200px,_1fr)_280px] 2xl:grid-cols-[140px_1fr_140px_1fr] 2xl:grid-rows-[65px_minmax(150px,_1fr)_280px] place-items-stretch'>
                ...
                <div className='bg-white 2xl:rounded-tr-2xl flex items-center justify-between border-b border-primary-800-light-16 px-6 lg:px-10'>
                  ...
                  <IconBoothListToggle
                    isActive={boothInfo?.isOpen}
                    onClick={() =>
                      isBoothOwner
                        ? handleClickBoothOpen()
                        : handleUnauthorizedAction()
                    }
                  />
                </div>
                ...
                <div className='min-w-full overflow-x-auto overflow-y-hidden 2xl:col-span-3 flex justify-between items-center bg-white rounded-br-[20px] border-primary-700 h-[280px] py-[40px] px-[40px] gap-[23px]'>
                  <div onClick={handleClickLeftIndicator}>
                    <IconIndicator left />
                  </div>
                  <div
          ...
          {ADMIN_CATEGORY[boothInfo.adminCategory] == 'night' && (
            <div className='flex flex-col gap-[20px] w-full'>
              ...
              <div className='w-full bg-primary-800-light-3 h-auto rounded-2xl px-[40px] py-[40px] border-1 border-primary-800-light-16'>
                <div className='w-full bg-primary-800-light-8 h-full rounded-2xl border border-primary-800-light-16 grid grid-cols-[120px_1fr] grid-rows-[60px_60px_60px] 2xl:grid-cols-[120px_1fr_200px_1fr] 2xl:grid-rows-[60px_60px] place-items-stretch'>
                  <div className='flex items-center justify-center border-b border-r border-primary-800-light-16 px-4 text-sm font-medium'>
                    예금주
                  </div>
                  <div className='bg-white flex items-center border-b border-primary-800-light-16 text-wrap px-6 lg:px-10 2xl:rounded-none rounded-tr-[20px] text-sm'>
                    {boothInfo.accountInfo?.accountHolder}
                  </div>
                  <div className='flex items-center justify-center 2xl:border-x border-r border-b border-primary-800-light-16 px-4 font-medium text-sm'>
                    은행명
                  </div>
                  <div className='bg-white 2xl:rounded-tr-2xl flex items-center justify-between border-b border-primary-800-light-16 px-6 lg:px-10 text-sm'>
                  ...
                  <div className='px-6 lg:px-10 2xl:col-span-3 bg-white flex items-center border-primary-800-light-16 rounded-br-2xl text-sm'>
          ...
          <div className='flex flex-col gap-[20px] w-full'>
            ...
            <div className='bg-primary-800-light-3 rounded-2xl w-full lg:py-[40px] lg:px-[40px] px-4 py-4 flex flex-col border-1 border-primary-800-light-16'>
              <div className='grid gap-4 grid-cols-1 2xl:grid-cols-2'>
                {menuList.length === 0 ? (
                  <div className='col-span-2 h-[220px] border-danger rounded-2xl border flex flex-col items-center justify-center bg-white'>
                    <div className='text-xl text-danger-800'>
                      메뉴를 등록해주세요.
                    </div>
                  </div>
                ) : (
          ...
          {ADMIN_CATEGORY[boothInfo.adminCategory] === 'night' && (
            <div className='flex gap-6 md:gap-[40px] items-center flex-wrap'>
              ...
              <div className='flex gap-[28px]'>
                <div className='flex items-center justify-center gap-2 flex-shrink-0 cursor-pointer'>
                  <IconRadio
                    isActive={boothInfo.isReservation}
                    readOnly={true}
                  />
                  <div
                    className={`text-sm font-semibold ${
                      boothInfo.isReservation
                        ? 'text-success-800'
                        : 'text-danger-800'
                    }`}
                  >
              ...
              {boothInfo.isKakaoPay && (
                <div className='flex gap-2 md:gap-4 items-center flex-wrap'>
                  ...
                  <div className='relative w-full'>
                    <div className='w-full py-3 border border-primary-800-light-16 rounded-xl px-[20px] text-sm break-all'>
                      {boothInfo?.kakaoPay ?? ''}
                    </div>
                  </div>
                </div>
