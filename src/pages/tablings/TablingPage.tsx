...
const TablingPage: React.FC = () => {
  ...
  const setIsUpdate = (reserveLength: number) => {
      if (isUserChecked) {
          ...
          isUpdate.current = reserveLength !== reserveNum.current
      }

      reserveNum.current = reserveLength
  }
  ...
  return (
      <div className='flex flex-col px-4 gap-[40px] min-w-[850px] pb-20'>
          ...
          {isAdmin && (
              <div className='flex justify-between min-w-[350px] items-center gap-4'>
                  <div className='flex-shrink-0 text-md font-semibold'>
                      학과 선택:
                  </div>
                  <select
                      className='bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5'
                      value={selectBoothId}
                      onChange={(e) => setSelectBoothId(e.target.value)}>
              ...
              <table className='w-full text-xs lg:text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400'>
                  <thead className='text-xs lg:text-sm uppercase bg-primary-50 border-b-1 border-secondary-200'>
                      <tr>
                          <th
                              scope='col'
                              className='px-6 py-3 text-center text-secondary-700-light font-medium capitalize truncate'>
                              No.
                          </th>
                          <th
                              scope='col'
                          ...
                          <th
                              scope='col'
                              className='px-6 py-3 text-center text-secondary-700-light font-medium truncate'>
                              예약자
                          </th>
                          <th
                              scope='col'
                  ...
                  <tbody className='text-xs lg:text-sm'>
                          ...
                          getFilteredReserveList(selectOrderType).map(
                              (reserve, index) => (
                                  <tr
                                      ...
                                      <th
                                          scope='row'
                                          className='px-4 py-4 whitespace-nowrap text-center font-normal'>
                                          {index + 1}
                                      </th>
                                      <th
                                          scope='row'
                      ...
                      {!isLoading &&
                          getFilteredReserveList(selectOrderType).length ===
                              0 &&
                          selectBooth?.isReservation && (
                              <tr>
